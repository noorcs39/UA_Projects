...
import math  # Add this import for the sqrt function
...
def compare_counts(exact_counts, approximate_counts):
    ...
    for count in approximate_counts:
        for word in exact_top_words:
            hash_value = hash(word)
            for k in range(1, int(math.log(len(count), 3)) + 1):
                index = hash_value % len(count)
                if count[index] == 1:
...
def main():
    ...
    approximate_counts = [approximate_counter(re.findall(r'\b\w+\b', text_content.lower()), probability=1/(3**0.5)) for _ in range(num_tests)]
