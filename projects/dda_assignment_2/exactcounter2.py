...
def approximate_counter(words):
    # Implement your approximate counting method here
    # For simplicity, this example uses the exact Counter as an approximate counter
...
def compare_counts(exact_counts, approximate_counts):
    ...
    approximate_top_words = [word for count in approximate_counts for word, _ in count.most_common(20)]
    ...
    for word in common_words:
        ...
        approximate_total_count = sum(count[word] for count in approximate_counts)
...
def main():
    ...
    approximate_counts = [approximate_counter(re.findall(r'\b\w+\b', text_content.lower())) for _ in range(num_tests)]
